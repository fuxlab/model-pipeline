...
import config.mask_rcnn.inference.utils as utils
import numpy as np
import math
...
class ForwardModel:
    ...
    def format_output(self, result_dict):
        mask_shape = result_dict.outputs[self.outputs['mask']].tensor_shape.dim
        mask_shape = tuple(d.size for d in mask_shape)
        mask = np.array(result_dict.outputs[self.outputs['mask']].float_val)
        mask = np.reshape(mask, mask_shape)

        detection_shape = result_dict.outputs[self.outputs['detection']].tensor_shape.dim
        detection_shape = tuple(d.size for d in detection_shape)
        detection = np.array(result_dict.outputs[self.outputs['detection']].float_val)
    ...
    def result_to_dict(self, images, molded_images, windows, result_dict):
        ...
        for i, image in enumerate(images):
            ...
            results.append({
                ...
                "class": final_class_ids.tolist(),
                "scores": final_scores.tolist(),
                "mask": final_masks.tolist(),
            })
        return results
